...
class ApiService {
  final String baseUrl =
      'http://192.168.0.114:8000'; // Replace with your server address

  Future<Map<String, dynamic>> signup(
      String username, String email, String password) async {
    final response = await http.post(
      Uri.parse('http://192.168.0.114:8000/signup/'),
...
// login
  ...
  Future<void> editUserProfile(String originalEmail, String newUsername, String imageUrl, String bio) async {
    ...
    final response = await http.put(
      Uri.parse('http://192.168.0.114:8000/profile/$originalEmail/'),  // Adjusted the URL here
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({
  ...
  Future<User> fetchUserProfile(String email) async {
    final response = await http.get(
      Uri.parse('http://192.168.0.114:8000/profile/$email/'),
    );
